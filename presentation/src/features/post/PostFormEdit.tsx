...
import { fetchPostAsync, postSelectors, setPost } from './postSlice';
...
export default function PostFormEdit() {
	...
	async function handleSubmitData(data: FieldValues) {
		try {
			...
			await agent.PostStore.updatePost(editPostDto)
				.then(() => {
					history.push(`/posts/${id}`);
					// dispatch(setPost())
					toast.done('Updated post successfully');
				})
			...
			<form onSubmit={handleSubmit(handleSubmitData, submitDataError)}>
				<Grid container spacing={3}>
					...
					<Grid item xs={12} sm={6}>
						<AppTextInput
							defaultValue={
								currentPost?.postLocation.detailedLocation
							}
							control={control}
